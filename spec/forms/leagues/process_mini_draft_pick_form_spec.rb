...
RSpec.describe Leagues::ProcessMiniDraftPickForm, type: :form do
  ...
  let(:fpl_team) { FactoryBot.create(:fpl_team, user: user, league: league, rank: 10) }
  let!(:fpl_team2) { FactoryBot.create(:fpl_team, league: league, rank: 9) }
  let!(:fpl_team3) { FactoryBot.create(:fpl_team, league: league, rank: 8) }
  before do
    3.times do
    ...
    FactoryBot.create(:round, name: 'Gameweek 1', deadline_time: 2.weeks.ago, is_current: false)
    ::FplTeams::ProcessInitialLineUp.run(fpl_team: fpl_team)
  end

  it 'successfully creates a mini draft pick' do
    round = FactoryBot.create(:round, name: 'Gameweek 2', deadline_time: 1.weeks.ago, is_current: false)
    mini_draft_round = FactoryBot.create(
      :round,
      name: 'Gameweek 3',
